...
const winStories = [
  ...
  (h: string, m: string) => `💥 ${h} unleashed a devastating combo — the ${m} crumbled under the pressure!`,
  (h: string, m: string) => `🏆 ${h} dominated the ${m} from start to finish. A flawless victory!`,
]
const loseStories = [
  (h: string, m: string) => `💀 The ${m} was too powerful — ${h} fought bravely but fell in battle.`,
  (h: string, m: string) => `🔥 The ${m}'s fury was overwhelming. ${h} needs to level up and try again!`,
  (h: string, m: string) => `😤 ${h} underestimated the ${m}. Train harder and return for revenge!`,
]
...
function RoomRow({ roomId, address, writeContract, isPending, setMsg, dark }: any) {
  ...
  const border = dark ? '#2d2d3a' : '#e5e7eb'
  const bg = dark ? '#13131f' : '#f9fafb'
  const codeBg = dark ? '#1e1e30' : '#f0f0ff'
  return (
    <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: '10px', padding: '12px 16px', marginBottom: '8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
        ...
        <button onClick={copyCode} style={{ background: copied ? (dark ? '#052e16' : '#ecfdf5') : (dark ? '#1e1e30' : '#f0f0ff'), color: copied ? '#10b981' : (dark ? '#818cf8' : '#4f46e5'), border: `1px solid ${copied ? '#10b981' : (dark ? '#3d3d6a' : '#c7d2fe')}`, borderRadius: '6px', padding: '6px 12px', fontSize: '12px', cursor: 'pointer', fontWeight: 600, whiteSpace: 'nowrap' as const }}>
...
function Game() {
  ...
  function navigate(target: string) { setPageHistory(h => [...h, page]); setPage(target); setMsg('') }
  function goBack() { if (!pageHistory.length) return; setPage(pageHistory[pageHistory.length-1]); setPageHistory(h => h.slice(0,-1)); setMsg('') }
  ...
  const statCard = (color: string): React.CSSProperties => ({ background: T.subtle, borderRadius: '10px', padding: '14px 16px', borderLeft: `3px solid ${color}` })

  const currentLabel = [...navItems, { id: 'profile', label: 'Profile', icon: '👤' }].find(n => n.id === page)?.label || 'Dashboard'

  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: T.bg, color: T.text, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', transition: 'background 0.2s, color 0.2s' }}>
        ...
        <header style={{ background: T.sidebar, borderBottom: `1px solid ${T.border}`, padding: '13px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            {pageHistory.length > 0 && (
              <button onClick={goBack} style={{ background: T.subtle, border: `1px solid ${T.border}`, borderRadius: '8px', width: '34px', height: '34px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: T.text, fontSize: '16px', flexShrink: 0 }}>←</button>
            )}
            <div>
        ...
        <main style={{ flex: 1, padding: '28px', overflowY: 'auto' }}>
          {!isConnected && !['dashboard','tournament'].includes(page) && (
            <div style={{ ...sec(), textAlign: 'center', padding: '80px' }}>
              ...
              <p style={{ color: T.muted, fontSize: '14px' }}>Use the button in the top-right corner to connect.</p>
            </div>
          )}
            ...
            <div>
              <div style={{ marginBottom: '24px' }}>
                <h1 style={{ fontSize: '24px', fontWeight: 800, color: T.text, margin: 0 }}>
                  {isConnected && myProfile?.exists ? `Welcome back, ${myProfile.username} 👋` : 'Welcome to Nexus RPG 👋'}
                </h1>
              ...
              {isConnected && (!hasHero || !hasProfile || !questProgress?.[4]) && (
                <div style={{ ...sec({ marginBottom: '20px', borderColor: dark ? '#3d3d6a' : '#c7d2fe' }) }}>
                  <div style={{ fontSize: '14px', fontWeight: 700, color: T.text, marginBottom: '4px' }}>🚀 Get started</div>
                  <div style={{ fontSize: '13px', color: T.muted, marginBottom: '16px' }}>Complete these steps to start playing.</div>
                  <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '10px' }}>
                    {[
                      { done: hasHero, label: 'Create your hero', sub: 'Name your hero and mint them onchain.', page: 'hero', cta: 'Create Hero →' },
                      { done: hasProfile, label: 'Set up your profile', sub: 'Choose a username and avatar.', page: 'profile', cta: 'Set Up Profile →' },
                      { done: Boolean(questProgress?.[4]), label: 'Claim your daily login bonus', sub: 'Log in every day to earn XP.', page: 'quests', cta: 'Claim Bonus →' },
                    ].map((s, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 16px', background: s.done ? (dark ? '#052e16' : '#f0fdf4') : T.bg, border: `1px solid ${s.done ? '#10b981' : T.border}`, borderRadius: '10px' }}>
                        ...
                        {!s.done && (
                          <button onClick={() => navigate(s.page)} style={{ background: T.accent, color: '#fff', border: 'none', borderRadius: '8px', padding: '8px 14px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' as const }}>{s.cta}</button>
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' as const, gap: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: '12px', color: T.muted, marginTop: '2px' }}>Level {heroData.level.toString()} · {heroData.totalXpEarned.toString()} XP earned</div>
                      </div>
                    </div>
                ...
                <div style={sec()}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '12px', color: T.muted, marginTop: '2px' }}>Enter a code or join any open room</div>
                    </div>
                    {isConnected && <button onClick={() => navigate('prizepool')} style={{ background: T.accent, color: '#fff', border: 'none', borderRadius: '8px', padding: '7px 14px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' as const }}>+ Create</button>}
                  </div>
                  <div style={{ display: 'flex', gap: '8px', marginBottom: '14px' }}>
                    <input value={manualCode} onChange={e => setManualCode(e.target.value.toUpperCase())} placeholder="Enter code" maxLength={4} style={{ ...inp({ marginBottom: 0, flex: 1, fontFamily: 'monospace', fontSize: '16px', letterSpacing: '3px', textAlign: 'center', fontWeight: 700 }) }} />
                  ...
                  {openRooms && (openRooms as bigint[]).length > 0
                    ...
                    : <div style={{ color: T.muted, fontSize: '13px', textAlign: 'center', padding: '20px', background: T.bg, borderRadius: '8px', border: `1px solid ${T.border}` }}>No open rooms — create one!</div>}
                </div>
                <div style={sec()}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                    <div>
                  ...
                  {leaderboard
                    ...
                    : <div style={{ color: T.muted, textAlign: 'center', padding: '24px', fontSize: '13px' }}>No players yet</div>}
                </div>
              </div>
              ...
              {isConnected && hasHero && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px' }}>
                  {[
                    { icon: '⚡', label: 'Battle a Monster', sub: 'Earn XP · level up · get stronger', color: '#ef4444', page: 'battle' },
                    { icon: '📋', label: 'Daily Quests', sub: questProgress?.[4] ? '✅ Login bonus claimed today' : '⏰ Login bonus available!', color: '#f59e0b', page: 'quests' },
          ...
          {page === 'hero' && isConnected && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              <div style={sec()}>
                ...
                <p style={{ color: T.muted, fontSize: '13px', marginBottom: '20px' }}>One hero per wallet. XP from battles and quests makes you stronger.</p>
              ...
              <div style={sec()}>
                ...
                ) : <div style={{ color: T.muted, textAlign: 'center', padding: '48px', fontSize: '14px' }}>No hero yet — create one!</div>}
              </div>
            </div>
          )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              <div>
                <div style={{ ...sec({ marginBottom: '16px' }) }}>
                  ...
                  <p style={{ color: T.muted, fontSize: '13px', marginBottom: '20px' }}>Pay 0.0001 ETH · Winner takes 90% · Share your 4-letter code with anyone.</p>
                  <button onClick={() => { writeContract({ address: PRIZEPOOL as any, abi: PRIZEPOOL_ABI, functionName: 'createRoom', value: ENTRY_FEE }); setMsg('Creating room...') }} disabled={isPending} style={btn(T.accent)}>
                    {isPending ? 'Creating...' : '+ Create Battle Room (0.0001 ETH)'}
                  </button>
                </div>
                <div style={{ ...sec({ marginBottom: '16px' }) }}>
                  <h3 style={{ color: T.text, marginBottom: '4px', fontSize: '16px' }}>Join by Code</h3>
                  <p style={{ color: T.muted, fontSize: '13px', marginBottom: '14px' }}>Got a code from a friend? Enter it here.</p>
                ...
                {pendingWin && BigInt(pendingWin.toString()) > 0n && (
                  <div style={sec()}>
                    <div style={{ fontSize: '14px', fontWeight: 600, color: T.text, marginBottom: '10px' }}>🎉 You have winnings!</div>
              ...
              <div>
                ...
                <div style={sec()}>
                  ...
                  {openRooms && (openRooms as bigint[]).length > 0
                    ? (openRooms as bigint[]).map(id => <RoomRow key={id.toString()} roomId={id} address={address} writeContract={writeContract} isPending={isPending} setMsg={setMsg} dark={dark} />)
                    : <div style={{ color: T.muted, fontSize: '13px', textAlign: 'center', padding: '20px' }}>No open rooms</div>}
                </div>
              </div>
            </div>
          )}
          ...
          {page === 'tournament' && (
            <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '20px' }}>
              <div style={sec()}>
                ...
                <div style={{ marginTop: '20px', background: T.bg, borderRadius: '8px', padding: '14px', border: `1px solid ${T.border}`, fontSize: '13px' }}>
                  <div style={{ color: T.muted, marginBottom: '8px', fontWeight: 600, fontSize: '11px', textTransform: 'uppercase' as const, letterSpacing: '0.08em' }}>Prize Split</div>
                  {[['🥇 1st', '50%'], ['🥈 2nd', '30%'], ['🥉 3rd', '20%']].map(([p,s]) => (
                    <div key={p} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', color: T.text }}>
              ...
              <div style={sec()}>
                ...
                {leaderboard
                  ...
                  : <div style={{ color: T.muted, textAlign: 'center', padding: '40px', fontSize: '13px' }}>No players yet</div>}
              </div>
            </div>
          )}
          ...
          {page === 'quests' && isConnected && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              <div style={sec()}>
                ...
                <p style={{ color: T.muted, fontSize: '13px', marginBottom: '20px' }}>Log in every day to build your streak and earn more XP!</p>
                <div style={{ background: T.bg, borderRadius: '10px', padding: '18px', marginBottom: '16px', border: `1px solid ${T.border}` }}>
                  {[['Login Streak', `${questProgress?questProgress[1].toString():'0'} days 🔥`, '#f59e0b'], ['Total Logins', questProgress?questProgress[2].toString():'0', T.text], ["Today's XP", `+${loginXp} XP`, T.accent]].map(([k,v,c]) => (
                ...
                <button onClick={() => { writeContract({ address: MASTER as any, abi: MASTER_ABI, functionName: 'claimDailyLogin' }, { onSuccess: () => { setTimeout(() => { refetchHero(); refetchXP(); refetchProgress() }, 2000); setMsg('Daily bonus claimed! ✅') } }); setMsg('Claiming...') }} disabled={isPending || Boolean(questProgress?.[4])} style={btn('#10b981', Boolean(questProgress?.[4]))}>
          ...
          {page === 'loot' && isConnected && (
            <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '20px' }}>
              <div style={sec()}>
                ...
                <p style={{ color: T.muted, fontSize: '13px', marginBottom: '20px' }}>Pay a small gas fee to roll a random item.</p>
              ...
              <div style={sec()}>
                ...
                {inventory && inventory.length > 0 ? (
                  ...
                  <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                    ...
                    <div style={{ color: T.muted, fontSize: '14px' }}>No items yet — roll for loot!</div>
                  </div>
                )}
              </div>
            </div>
          ...
          {page === 'profile' && isConnected && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              <div style={sec()}>
                ...
                <p style={{ color: T.muted, fontSize: '13px', marginBottom: '20px' }}>Your username appears on rooms, leaderboards and throughout the game.</p>
                {!myProfile?.exists && (
                  <>
                ...
                <span style={lbl}>𝕏 Handle (optional)</span>
              ...
              <div style={sec()}>
                ...
                {myProfile?.exists ? (
                  <div>
                    ...
                    {heroData?.name && (
                      <div style={{ background: T.bg, borderRadius: '8px', padding: '14px 16px', border: `1px solid ${T.border}` }}>
